...
export async function getUsageStats(): Promise<UsageStats> {
  ...
  const [wallets] = await rows(
    sql`select count(distinct public_key)::int c from hatiin_sessions where public_key not in ${demo}`,
  );
  const [logins] = await rows(
    sql`select count(*)::int c from hatiin_sessions where public_key not in ${demo}`,
  );
  const [billsTotal] = await rows(
    sql`select count(*)::int c from hatiin_bills where creator_public_key not in ${demo}`,
  );
  const [billsSettled] = await rows(
    sql`select count(*)::int c from hatiin_bills where creator_public_key not in ${demo} and status = 'settled'`,
  );
  const [partsTotal] = await rows(
    sql`select count(*)::int c from hatiin_participants p
        join hatiin_bills b on b.id = p.bill_id
